...
def inference(inputs, meanshape, pts_num, is_training=True, dropout_keep_prob=0.5, scope='tiny', global_pool=False):
    ...
    # with tf.variable_scope(scope, default_name='tiny', values=[inputs], reuse=True) as sc:
    # with tf.variable_scope(None, default_name=None, values=[inputs], reuse=True) as sc:
